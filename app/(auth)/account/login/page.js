...
const loginSchema = yup.object().shape({
  email: yup.string().required("Login / E-mail jest wymagany").min(3, "Login musi mieć minimum 3 znaki"),
  password: yup.string().required("Hasło jest wymagane").min(6, "Hasło musi mieć minimum 6 znaków"),
});
    ...
    <div className="max-w-md mx-auto px-4 sm:px-6 py-12 animate-fade-in">
      <div className="glass-panel p-8 rounded-2xl space-y-6 shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleLogin} className="space-y-4" noValidate>
          ...
          >
            {loading ? (<><Loader2 className="w-4 h-4 animate-spin" />Logowanie...</>) : (t("navLogin"))}
          </button>

          <div className="text-center text-xs text-slate-500">
            Nie masz jeszcze konta?{" "}
            <Link href="/account/signup" className="text-brand-red font-bold hover:underline">Zarejestruj się</Link>
          </div>
        </form>
      </div>
    </div>
