  ...
  useEffect(() => {
    if (urlId) {
      ...
      const match = bookings.find(
        (b) => b.id.toUpperCase() === urlId.toUpperCase().trim(),
      );
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-sm text-slate-500">
          Sprawdź szczegóły, status rezerwacji oraz termin odbioru wprowadzając
          numer rezerwacji.
        </p>
      </div>
          ...
          {activeBooking ? (
            <div className="glass-panel rounded-2xl p-6 space-y-6 shadow-sm border border-slate-100 relative overflow-hidden">
              <div className="absolute top-0 right-0">
                <span
                  className={`inline-block px-4 py-1.5 rounded-bl-xl text-xs font-black uppercase text-white ${
              ...
              <div className="border-b border-slate-100 pb-4">
                <span className="text-[10px] text-slate-400 tracking-widest font-bold uppercase">
                  {t("confirmNum")}
                </span>
                <h2 className="text-2xl font-black text-slate-800 font-mono tracking-wider mt-0.5">
                  {activeBooking.id}
                </h2>
              </div>
              ...
              <div className="flex items-start space-x-2.5 p-3.5 bg-slate-50 border border-slate-100 rounded-xl text-xs text-slate-500">
                ...
                <div>
                  <p className="font-semibold text-slate-700">
                    Weryfikacja tożsamości aktywna
                  </p>
                  ...
                  <div className="flex justify-between items-center text-slate-800">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Klasa: {activeBooking.car.class} | Paliwo:{" "}
            ...
            <div className="p-8 border border-brand-red/20 rounded-2xl bg-brand-red/5 text-center text-sm font-semibold text-brand-red space-y-2.5 animate-pulse">
              ...
              <p className="text-xs text-slate-500">
                Upewnij się, że wpisany identyfikator jest poprawny (np.
                CAR-GO-XXXX).
              </p>
            </div>
          )}
