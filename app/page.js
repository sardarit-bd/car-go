...
import { AlertCircle, Briefcase, CheckCircle, Settings, Sparkles, Star, Users } from "lucide-react";
...
export default function Home() {
  ...
  // Filter approved reviews for bottom section
  const approvedReviews = reviews.filter((r) => r.approved).slice(0, 3);

  // Render a responsive, detailed SVG illustration of a modern luxury car for hero section background/graphics
  const renderCarSvg = () => (
    <svg className="w-full max-w-lg mx-auto opacity-80 drop-shadow-[0_10px_20px_rgba(255,0,0,0.1)]" viewBox="0 0 500 220" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M120 125 C 170 120, 260 120, 280 105 L 340 105 C 375 120, 390 125, 430 125" stroke="#FF0000" strokeWidth="3.5" strokeLinecap="round" /> {/* Red Accent Line */}
      ...
      <path d="M280 102 L 315 72 C 322 65, 335 63, 345 68 L 388 90 C 395 93, 398 100, 395 107 Z" fill="#0f172a" stroke="#cbd5e1" strokeWidth="1.5" /> {/* Window */}
    </svg>
  );
    ...
    <div className="space-y-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center w-full">
          ...
          <div className="lg:col-span-5 space-y-6 text-center lg:text-left animate-slide-up">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight leading-tight">
              <span className="text-gradient block">{getCmsText("homeHeader", "WYNAJEM SAMOCHODÓW")}</span>
              <span className="text-gradient-red italic block mt-1">CAR-GO.PL</span>
            </h1>
      ...
      <section className="px-4 sm:px-6 max-w-7xl mx-auto">
        ...
        <div className="text-center mt-10">
          ...
          >
            <span>ZOBACZ WSZYSTKIE POJAZDY / VIEW ALL VEHICLES</span>
            <span>→</span>
          </Link>
        </div>
      </section>
      ...
      {/* Minimum Rental Rules Banner */}
      <section className="px-4 sm:px-6 max-w-5xl mx-auto">
        <div className="p-6 bg-yellow-50 border border-yellow-100 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center space-x-3.5 text-left">
            <AlertCircle className="w-8 h-8 text-yellow-600 flex-shrink-0" />
            <div>
              <h3 className="text-sm font-bold text-yellow-800">Weryfikacja Minimalnego Czasu Wynajmu</h3>
              <p className="text-xs text-slate-600 leading-relaxed mt-0.5 font-medium">
                Nasz system automatycznie kontroluje minimalny okres najmu. Dla rejonów Oława oraz Grodków minimalny okres najmu wynosi 5 dni. Baza Skarbimierz oraz miasto Brzeg pozwalają na rezerwacje 1-dniowe.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3.5 h-3.5 ${i < rev.rating ? "text-yellow-500 fill-yellow-500" : "text-slate-300"
                        }`}
                    />
                  ))}
                </div>
                <p className="text-xs text-slate-600 italic leading-relaxed font-semibold">
                  "{rev.text}"
                </p>
              </div>
