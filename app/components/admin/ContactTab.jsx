...
export default function ContactTab() {
  const { isOwner, fetchUnreadContactCount } = useApp();
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase border ${styles[status] || styles.PENDING}`}
      >
        {status || "PENDING"}
      </span>
    );
                  ...
                  <div className="flex md:flex-col gap-2 shrink-0">
                    {msg.status === "PENDING" && (
                      <button
                        ...
                        className="px-3 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 rounded-lg transition flex items-center justify-center gap-1.5 text-xs font-bold"
                      >
                        <CheckCircle className="w-3.5 h-3.5" /> Oznacz jako
                        przeczytane
                      </button>
                    )}
                    {msg.status !== "SOLVED" && msg.status !== "CLOSED" && (
                      <button
                        onClick={() => handleStatusUpdate(msg.id, "SOLVED")}
                        className="px-3 py-2 bg-green-50 hover:bg-green-100 border border-green-200 text-green-700 rounded-lg transition flex items-center justify-center gap-1.5 text-xs font-bold"
                      >
                        <CheckCircle className="w-3.5 h-3.5" /> Rozwiązane
                      </button>
                    )}
                    <a
                      href={`mailto:${msg.email}?subject=${encodeURIComponent("Re: your message to CAR-GO.PL")}`}
                      className="px-3 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-lg transition flex items-center justify-center gap-1.5 text-xs font-bold"
                    >
                      <Mail className="w-3.5 h-3.5" /> Odpowiedz
                    </a>
                  </div>
                </div>
              </div>
