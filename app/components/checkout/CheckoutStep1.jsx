...
function renderCarSvg() {
  return (
    <svg
      className="w-20 h-10 opacity-80"
      viewBox="0 0 500 220"
      fill="none"
    ...
    <div className="space-y-6">
      {!activeSearch && (
        <div className="p-5 bg-blue-50 border border-blue-200 rounded-2xl text-sm text-blue-900 font-semibold flex items-start gap-3">
          ...
          <span>
            Proszę wyszukać dogodne terminy przy użyciu formularza. Wyświetlamy
            ofertę domyślną.
          </span>
        </div>
      )}
        ...
        <>
          <div className="flex items-center justify-between pb-4 border-b border-slate-200">
            ...
            <span className="text-sm font-bold text-slate-500">
              ...
              {availableVehicles.length === 1 ? "pojazd" : "pojazdów"}
            </span>
          </div>
            ...
            {availableVehicles.length === 0 ? (
              <div className="p-12 text-center bg-white border border-slate-200 rounded-3xl space-y-3 shadow-sm">
                ...
                <p className="text-slate-500 font-semibold">
                  Wszystkie nasze samochody są zarezerwowane w wybranym
                  terminie.
                </p>
              </div>
            ) : (
              availableVehicles.map((car) => {
                const carTotal = activeSearch?.isCustomPrice
                  ? "Individual Price"
                  : car.price * getDays();
                return (
                  <div
