...
export default function MyReservationsUserBookings({ myBookings, currentUser, lang }) {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-4 shadow-sm no-print">
      ...
      {myBookings.length > 0 ? (
        ...
        <p className="text-sm text-slate-500 py-2">Brak aktywnych rezerwacji dla konta {currentUser.email}.</p>
      )}
    </div>
  );
}